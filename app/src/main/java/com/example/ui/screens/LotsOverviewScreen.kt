...
fun LotsOverviewScreen(
    ...
    val activeLotsWithProduct = productsWithLots.flatMap { pWithLots ->
        pWithLots.lots.filter { it.quantity > 0.001 }.map { lot ->
            Pair(pWithLots.product, lot)
        }
    }.sortedBy { it.second.expirationDate }

    val expiredLots = activeLotsWithProduct.filter { it.second.daysUntilExpiration(now) < 0 }
        ...
        item {
            Row(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    FilledTonalButton(
                        onClick = {
                            val pdf = com.example.utils.PdfReportService.generateStockReportPdf(
                                context = context,
                                products = productsWithLots,
                                filterName = "Controle de Validades (FEFO)"
                            )
                            if (pdf != null) {
